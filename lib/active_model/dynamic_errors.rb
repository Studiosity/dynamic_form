      ...
      map do |error|
        if error.attribute == :base
          ...
          if error.message =~ /^\^/
            ...
            I18n.t(:"errors.dynamic_format", options.merge(message: error.message[1..-1]))
          elsif error.message.is_a? Proc
            options[:default] = "%{message}"
            I18n.t(:"errors.dynamic_format", options.merge(message: error.message.call(@base)))
          else
            I18n.t(:"errors.format", options.merge(message: error.message))
          end
        end
      end
    end
